...
def register(app, deps):
    ...
    def _by_slug(db, slug):
        """The event this public link points at, and whether the link is dead.

        A retired link is not the same dead end as a typo. "We can't find
        that" tells somebody they got it wrong; "this has expired" tells them
        we replaced it, which is the truth and the thing that stops somebody
        retyping it three times before giving up.
        """
        ev = db.query(Event).filter(Event.slug == slug).first()
        if ev:
            return ev, False
        for e in (db.query(Event).filter(Event.old_slugs.isnot(None))
                  .filter(Event.old_slugs.contains(slug)).all()):
            if slug in e.retired_slugs():
                return e, True
        return None, False
    ...
    def signup_page(request: Request, slug: str, db: Session = Depends(get_db)):
        ...
        ev, retired = _by_slug(db, slug)
        if retired:
            return templates.TemplateResponse(
                "event_gone.html", {"request": request, "reason": "expired"},
                status_code=410)
    ...
    async def signup_start(request: Request, slug: str,
        ...
        """
        # A retired link cannot post either: the form is gone from
        # the page, but a stale tab could still submit against it.
        ev, retired = _by_slug(db, slug)
        if retired:
            ev = None
        if not ev or ev.mode != EVENT_OPEN:
        ...
        if want and want != ev.slug:
            ...
            if taken:
                ...
                return RedirectResponse(f"/events/{eid}/settings?err=slug",
                                        status_code=303)
            # Typed by hand or generated by the button, a replaced link is a
            # replaced link: the old one is remembered so it can say it
    ...
    @app.post("/events/{eid}/link/new")
    def event_new_link(request: Request, eid: int,
                       db: Session = Depends(get_db)):
        """Retire the public link and take a fresh one.

        For a class that has been shared further than you meant, or moved and
        re-opened to a different set of people. The old address stops working
        the moment this is pressed and says so — "this link has expired" —
        rather than pretending it was never real, because it was, and somebody
        holding it did nothing wrong.

        Only the public front door changes. Everybody already on the list keeps
        their own page, because that is reached by their code, not by this.
        """
        staff, redir = guard(request, db)
        if redir:
            return redir
        ...
        if not ev:
            return RedirectResponse("/events", status_code=303)
        base = slugify(ev.name) or "event"
        # Short and random rather than counted up: -2 on the end of a link
        # tells anybody holding it that a -1 existed, and invites them to try.
        for _ in range(40):
            fresh = "%s-%s" % (base, secrets.token_hex(3))
            if not db.query(Event).filter(Event.slug == fresh).first():
                ev.retire_slug(fresh)
                db.commit()
                return RedirectResponse(
                    "/events/%d/settings?newlink=1" % eid, status_code=303)
        return RedirectResponse("/events/%d/settings" % eid, status_code=303)
...
def rate_fill(ev) -> list:
    ...
    top of the page and "Doubles 12/15" underneath it are two readings of one
