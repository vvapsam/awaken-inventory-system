...
# --- TEMP one-off pricing setup (obscure path; removed right after use) ---
_AWK_SETUP_KEY = "GO-3f9k2q"


@app.get("/__awk_setup_3f9k2q")
def _awk_setup_inspect(k: str = "", db: Session = Depends(get_db)):
    if k != _AWK_SETUP_KEY:
        return JSONResponse({"ok": False}, status_code=404)
    prods = [{"id": p.id, "name": p.name, "price": float(p.selling_price or 0),
              "active": bool(p.is_active)}
             for p in db.query(Product).order_by(Product.name).all()]
    ents = [{"id": e.id, "name": e.name, "type": e.person_type or "",
             "level": e.pricing_group_id, "active": bool(e.is_active)}
            for e in db.query(Staff).order_by(Staff.name).all()]
    levels = [{"id": g.id, "name": g.name} for g in db.query(PricingGroup).all()]
    return {"ok": True, "products": prods, "entities": ents, "levels": levels}


@app.post("/__awk_setup_3f9k2q")
async def _awk_setup_apply(request: Request, db: Session = Depends(get_db)):
    data = await request.json()
    if data.get("k") != _AWK_SETUP_KEY:
        return JSONResponse({"ok": False}, status_code=404)
    report = {"levels": {}, "prices": [], "assigned": [], "unmatched": []}

    def get_level(name):
        g = db.query(PricingGroup).filter(func.lower(PricingGroup.name) == name.lower()).first()
        if not g:
            g = PricingGroup(name=name, kind="level", discount_percent=0)
            db.add(g); db.flush()
        return g

    emp = get_level("Employee")
    aff = get_level("Affiliate")
    report["levels"] = {"Employee": emp.id, "Affiliate": aff.id}

    # prices: [{ids:[pid...], employee:<price|None>, affiliate:<price|None>}]
    for spec in data.get("prices", []):
        for pid in spec.get("ids", []):
            p = db.get(Product, pid)
            if not p:
                report["unmatched"].append({"product_id": pid})
                continue
            for g, key in ((emp, "employee"), (aff, "affiliate")):
                val = spec.get(key)
                if val is None:
                    continue
                row = db.query(PricingGroupItem).filter_by(group_id=g.id, product_id=pid).first()
                if not row:
                    row = PricingGroupItem(group_id=g.id, product_id=pid)
                    db.add(row)
                row.price = val
            report["prices"].append({"pid": pid, "name": p.name,
