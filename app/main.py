...
def _dbcopy_deser(v):
    ...
    if isinstance(v, dict):
        ...
        if "__dt__" in v:
            s = v["__dt__"]
            return datetime.datetime.fromisoformat(s) if ("T" in s or " " in s) else datetime.date.fromisoformat(s)
        if "__dec__" in v:
            return decimal.Decimal(v["__dec__"])
    return v


@app.get("/admin/__export_all_q7fz3k")
def _export_all(db: Session = Depends(get_db)):
    # TEMPORARY read-only: dump every table (base64 for binary) for a prod->dev copy.
    out = {}
    for t in Base.metadata.sorted_tables:
        rows = []
        for r in db.execute(t.select()).mappings():
            rows.append({k: _dbcopy_ser(v) for k, v in dict(r).items()})
        out[t.name] = rows
    return out


def _dbcopy_load(data):
    """Overwrite THIS database's tables with the given export dict. Destructive."""
    counts = {}
    names = [t.name for t in Base.metadata.sorted_tables]
    with engine.begin() as conn:
        conn.exec_driver_sql("SET session_replication_role = 'replica'")
        conn.exec_driver_sql(
            "TRUNCATE TABLE " + ",".join('"%s"' % n for n in names) + " RESTART IDENTITY CASCADE")
        for t in Base.metadata.sorted_tables:
            rows = data.get(t.name, [])
            for row in rows:
                conn.execute(t.insert().values(**{k: _dbcopy_deser(v) for k, v in row.items()}))
            counts[t.name] = len(rows)
        for t in Base.metadata.sorted_tables:
            if "id" in t.columns:
                conn.exec_driver_sql(
                    "SELECT setval(pg_get_serial_sequence('%s','id'), "
                    "GREATEST(COALESCE((SELECT MAX(id) FROM \"%s\"),1),1))" % (t.name, t.name))
        conn.exec_driver_sql("SET session_replication_role = 'origin'")
    return counts


@app.get("/admin/__pull_from_prod_q7fz3k")
def _pull_from_prod():
    # TEMPORARY: DESTRUCTIVE. Fetch prod's export and overwrite THIS db with it.
    # Hard refuse on the production environment so this can never wipe prod.
    if os.environ.get("RAILWAY_ENVIRONMENT_NAME", "").lower() == "production":
        return JSONResponse({"ok": False, "error": "refused: this is production"}, status_code=403)
    import json, urllib.request
    URL = "https://pay.awakengym.com/admin/__export_all_q7fz3k"
    with urllib.request.urlopen(URL, timeout=120) as resp:
        data = json.loads(resp.read().decode())
    return {"ok": True, "imported": _dbcopy_load(data)}
